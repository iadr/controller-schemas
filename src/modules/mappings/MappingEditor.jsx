  ...
  return <form className="ui-form" onSubmit={submit}>
    {control.position && <label className="ui-field">Aplicar mapeo
      <select className="ui-input" value={scope} onChange={event => setScope(event.target.value)}>
        <option value="button">Este botón en este dispositivo</option>
        <option value="position">Cardinalidad {control.position} en todos los mandos</option>
      </select>
    </label>}
    {scope === 'position' && <p className="ui-muted">Se comparte en este contexto. Las excepciones por botón de otros mandos conservan prioridad.</p>}
    ...
    {!rows.length && <p className="ui-muted">Sin acciones. Guardar quitará las asociaciones de este control en el alcance seleccionado.</p>}
    <button className="ui-button" type="button" disabled={!unused.length}
      onClick={() => setRows(prev => [...prev, { type: unused[0], action: '' }])}>Agregar acción</button>
